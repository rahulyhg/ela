    ...
    $.fn.amazingaudioplayer = function(options) {
        var PlayerSkin = function(amazingPlayer, container, options, id) {
			...
			if (this.options.showtracklist) {
                ...
                for (var i = 0; i < this.amazingPlayer.elemLength; i++) {
                    ...
                    $track.data("order", i);
                    $track.html(t + "<hr/>");
                    $track.appendTo(this.$tracks).css({
					});
                ...
                this.container.append("<div class='amazingaudioplayer-tracklist-clear'></div><br/><br/><br/>")
            }
			//end of track list
            ...
            $bar.appendTo(this.container).css({
				"position": "fixed",
				"bottom": "0",
				"width": this.options.bottomfixwidth,
				"z-index": "999",
				"left": 0,
				"padding": "8px 0 8px 0"
			});
			...
			if (this.options.showtitle ||
                ...
                if (this.options.showinfo) {
                    ...
                    this.$title.appendTo($bar);
                    this.container.bind("amazingaudioplayer.updateinfo", function(event, data) {
                        var title = instance.options.titleformat.replace(/%TITLE%/g, data.title);
                        instance.$title.html(title);
                    });
					
					this.$album = $("<div class='amazingaudioplayer-album'></div>");
                    this.$album.appendTo($bar);
                    this.container.bind("amazingaudioplayer.updateinfo", function(event, data) {
                        var album= instance.options.titleformat.replace(/%TITLE%/g, data.album);
                        instance.$album.html(album);
                    });
					
					this.$artist = $("<div class='amazingaudioplayer-artist'></div>");
                    this.$artist.appendTo($bar);
                    this.container.bind("amazingaudioplayer.updateinfo", function(event, data) {
                        var artist = instance.options.titleformat.replace(/%TITLE%/g, data.artist);
                        instance.$artist.html("<i>By " + artist + "</i>");
                    });
					
					this.$contrartist = $("<div class='amazingaudioplayer-contribution-artist'></div>");
                    this.$contrartist.appendTo($bar);
                    this.container.bind("amazingaudioplayer.updateinfo", function(event, data) {
                        var t = instance.options.titleformat.replace(/%TITLE%/g, data.info);
                        //instance.$contrartist.html(t);
                    });
                }
            }
        ...
        AmazingAudioPlayer.prototype = {
            ...
            init: function(instance) {
				...
				console.log(instance.elemLength);
